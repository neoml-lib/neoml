...
class TransformerDecoder(Layer):
    """The transformer decoder layer based on the "Attention is all you need" article.
    ...
    :param hidden_size: The hidden size of self-attention layer, must be a multiple of head_count
    :type hidden_size: int, default=1
    :param dropout: The rate of the dropouts
    :type dropout: float, default=0.
    :param feed_forward_size: The size of the first fully-connected layer in feed-forward
    :type feed_forward_size: int, default=1
    :param activation: activation used between fully-connected layers in feed-forward
    ...
    (2) encoder data: the set of vector sequences, of dimensions:
    ...
    (3) (optional) input mask: blob containing 1.0 and 0.0 where 1.0 means IGNORED object, of dimensions:

        - **Width** (seq_Q) and **Channels** (seq_V) must be equal to the ListSize of the first input
        - Other dimensions must be equal to 1

    (4) (optional) encoder mask: blob containing 1.0 and 0.0 where 1.0 means IGNORED object, of dimensions:

        - **Width** (seq_Q) must be equal to the ListSize of the first input
        - **Channels** (seq_V) must be equal to the ListSize of the second input
        - Other dimensions must be equal to 1
    ...
    def __init__(self, input_layers, head_count=1, hidden_size=1, dropout=0., feed_forward_size=1, activation='relu', name=None):
        ...
        if type(input_layers) is PythonWrapper.TransformerDecoder:
            super().__init__(input_layers)
            return
        ...
        layers, outputs = check_input_layers(input_layers, (2, 4))

        internal = PythonWrapper.TransformerDecoder(str(name), layers, outputs,
            int(head_count), int(hidden_size), float(dropout), int(feed_forward_size), int(activation))
        super().__init__(internal)
