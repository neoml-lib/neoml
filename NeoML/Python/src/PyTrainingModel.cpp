...
class CPyOneVersusAll : public CPyTrainingModel {
public:
	explicit CPyOneVersusAll( CPyTrainingModelOwner* owner ) : CPyTrainingModel( new COneVersusAll( owner->TrainingModel() ) ), base( owner ) {}
private:
	CPtr<CPyTrainingModelOwner> base;
};
...
//------------------------------------------------------------------------------------------------------------
	...
	py::class_<CPyOneVersusAll, CPyTrainingModel>(m, "OneVersusAll")
		.def( py::init([]( const CPyTrainingModel& base_classifier ) { return new CPyOneVersusAll( base_classifier.GetOwner() ); }) )
		.def( "train_classifier", &CPyOneVersusAll::TrainClassifier, py::return_value_policy::reference )
	;
}
