...
public:
	...
	void SetBlob( const CPyDnnBlob& blob )
	{
		Layer<CSourceLayer>()->SetBlob( &blob.Blob() );
	}

	CPyDnnBlob GetBlob() const
	{
		return CPyDnnBlob( MathEngineOwner(), *Layer<CSourceLayer>()->GetBlob() );
	}
};
...
void InitializeSourceLayer( py::module& m )
{
	py::class_<CPySourceLayer, CPyLayer>(m, "Source")
		...
		.def( py::init([]( const CPyDnn& dnn, const std::string& name )
		{
			...
			sourceLayer->SetName( name == "" ? findFreeLayerName( dnn.Dnn(), "Source" ).c_str() : name.c_str() );
