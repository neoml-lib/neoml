...
void CDistributedTraining::serializeDnn( CDnn& dnn, int count, TDistributedInitializer initializer, int seed )
{
	CMemoryFile file;
	...
	threadPool( CreateThreadPool( count ) )
{
	...
	serializeDnn( const_cast<CDnn&>( dnn ), count, initializer, seed );
	...
	threadPool( CreateThreadPool( cudaDevs.Size() ) )
{
	...
	serializeDnn( const_cast<CDnn&>( dnn ), cudaDevs.Size(), initializer, seed );
	threadParams = new CThreadParams( /*isCpu*/true, cnns );
}
...
struct CDistributedInference::CThreadParams final {
	...
	CThreadParams( int threadsCount, CReferenceDnnFactory& factory );

	void SetData( IDistributedDataset* data = nullptr );
	void SetErrorMessage( int threadIndex, CString message );
};

CDistributedInference::CThreadParams::CThreadParams( int threadsCount, CReferenceDnnFactory& factory )
{
	initThreadGroupInfo();

	ErrorMessages.Add( CString{}, threadsCount );
	...
	for( int i = 1; i < threadsCount; ++i ) {
		Refs.Add( factory.CreateReferenceDnn() );
	}
	...
	Refs.Add( factory.CreateReferenceDnn( /*getOriginalDnn*/true ) );
}

void CDistributedInference::CThreadParams::SetData( IDistributedDataset* data )
{
	IsErrorHappened = false;
	Data = data;
}
