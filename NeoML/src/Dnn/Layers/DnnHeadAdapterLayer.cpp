...
void CDnnHeadAdapterLayer::processBackwardOrLearn()
{
	...
	if(head->headCounter == head->connections.Size() - 1) {
		for(int i = 0; i < head->dnn->layers.Size(); ++i) {
			if(head->dnn->layers[i]->IsLearningPerformed()) {
				int& layerCounter = GetDnn()->GetSolver()->layerToParamDiffBlobsSum.GetOrCreateValue(head->dnn->layers[i]).Count;
				layerCounter = layerCounter - head->connections.Size() + 1;
			}
		}
	}
	head->increment();
