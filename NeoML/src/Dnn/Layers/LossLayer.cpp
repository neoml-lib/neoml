...
{
	CheckInputs();
	CheckArchitecture( inputDescs.Size() >= 2, GetPath(), "loss layer with 1 input" );
	CheckArchitecture( inputDescs.Size() <= 3, GetPath(), "loss layer with more than 3 inputs" );
	CheckArchitecture( outputDescs.IsEmpty(), GetPath(), "loss layer has no output" );
	CheckArchitecture( inputDescs[0].ObjectCount() == inputDescs[1].ObjectCount(), GetPath(), "object count mismatch" );
	CheckArchitecture( !(trainLabels && inputDescs[1].GetDataType() == CT_Int), GetPath(), "can't train integer labels" );

	if( inputDescs.Size() > 2 ) {
		CheckArchitecture( inputDescs[0].BatchWidth() == inputDescs[2].BatchWidth(),
			GetPath(), "weights batch width doesn't match result batch width" );
	}
