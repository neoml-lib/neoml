...
void CCtcLossLayer::Reshape()
{
	...
	CheckArchitecture(outputDescs.IsEmpty(), GetPath(), "CCtcLossLayer has no output");
	...
	CheckArchitecture( GetInputCount() >= 2 && GetInputCount() <= 5,
		GetPath(), "CCtcLossLayer must have two to five inputs" );
	...
	if( hasLabelsLengths ) {
		CheckArchitecture( inputDescs[I_LabelsLengths].BatchLength() == 1 &&
			...
			GetPath(), "CCtcLossLayer: incorrect labels lengths blob dimensions" );
	}
	if( hasInputLengths ) {
		CheckArchitecture( inputDescs[I_InputLengths].BatchLength() == 1 &&
			...
			GetPath(), "CCtcLossLayer: incorrect inputs lengths blob dimensions" );
	}
...
{
	CheckInputs();
	CheckArchitecture(outputDescs.IsEmpty(), GetPath(), "CCtcDecodingLayer has no output");
