...
static const float ctcLossMaxGradientValue = 1e+6;
...
CCtcLossLayer::CCtcLossLayer( IMathEngine& mathEngine ) :
	...
	lossDivider( 0.f ),
	minGradient( -ctcLossMaxGradientValue ),
	maxGradient( ctcLossMaxGradientValue ),
	blankLabel( 0 ),
...
void CCtcLossLayer::Serialize( CArchive& archive )
{
	...
	if( archive.IsStoring() ) {
		...
		archive >> tmp;
		SetLossWeight(tmp);
		float maxGradientValue = ctcLossMaxGradientValue;
