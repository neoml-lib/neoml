...
void CAccuracyLayer::Reshape()
{
	...
	CheckArchitecture( inputDescs[0].Height() == 1, GetPath(), "input[0].Height() != 1" );
	CheckArchitecture( inputDescs[0].Width() == 1, GetPath(), "input[0].Width() != 1" );
	CheckArchitecture( inputDescs[0].Depth() == 1, GetPath(), "input[0].Depth() != 1" );

	outputDescs[0] = CBlobDesc( CT_Float );
	CheckArchitecture( inputDescs[0].ObjectSize() == inputDescs[1].ObjectSize()
		|| inputDescs[1].ObjectSize() == 1, GetPath(), "Second input object size must match or be equal to 1" );
	iterationsCount = 0;
	collectedAccuracy = 0;
}
...
{
	CheckInputs();
	CheckArchitecture( inputDescs.Size() == 2, GetPath(), "inputs.Size() != 2" );

	// For classifying a sigmoid a special implementation is needed
	CheckArchitecture( inputDescs[0].Channels() >= 2, GetPath(), "input[0].Channels() < 2");
	CheckArchitecture( inputDescs[0].Height() == 1, GetPath(), "input[0].Height() != 1" );
	CheckArchitecture( inputDescs[0].Width() == 1, GetPath(), "input[0].Width() != 1" );
	CheckArchitecture( inputDescs[0].ObjectCount() == inputDescs[1].ObjectCount(), GetPath(), "input[0].ObjectCount() != input[1].ObjectCount()" );
	CheckArchitecture( inputDescs[0].ObjectSize() >= 1, GetPath(), "input[0].ObjectSize() < 1" );
	CheckArchitecture( inputDescs[0].ObjectSize() == inputDescs[1].ObjectSize(), GetPath(), "input[0].ObjectSize() != input[1].ObjectSize()" );
