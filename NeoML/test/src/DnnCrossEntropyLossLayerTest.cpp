...
TEST_P( CCrossEntropyLossLayerTest, BinaryCrossEntropyLossSignTest )
{
	...
	CSourceLayer* result = Source( dnn, "result" );
	CSourceLayer* target = Source( dnn, "target" );

	CPtr<CBinaryCrossEntropyLossLayer> loss = BinaryCrossEntropyLoss()( result, target );

	CPtr<CDnnBlob> resultBlob = CDnnBlob::CreateDataBlob( MathEngine(), CT_Float, 1, 1, 1 );
	resultBlob->Fill( params.Result );
	result->SetBlob( resultBlob );

	CPtr<CDnnBlob> targetBlob = CDnnBlob::CreateDataBlob( MathEngine(), CT_Float, 1, 1, 1 );
	targetBlob->Fill( params.Target );
