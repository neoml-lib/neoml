...
TEST( Arm64LogTest, Log )
{
	CFloatHandleVar input( MathEngine(), 3 );
	{
		std::vector<float> inputData = { 1.f - FLT_EPSILON, 1.f, 1.f + FLT_EPSILON };
		MathEngine().DataExchangeTyped<float>( input.GetHandle(), inputData.data(), inputData.size() );
	}
	CFloatHandleVar output( MathEngine(), input.Size() );
	MathEngine().VectorLog( input.GetHandle(), output.GetHandle(), input.Size() );
	std::vector<float> outputData( input.Size() );
	MathEngine().DataExchangeTyped<float>( outputData.data(), output.GetHandle(), outputData.size() );
	for( const float& x : outputData ) {
		::printf( "%.10f\t%#010x\n", x, *reinterpret_cast<const uint32_t*>( &x ) );
	}
}

TEST( DebugArm64, ReadBlob )
{
	CPtr<CDnnBlob> blob;
	
	auto load = [&blob] ( const char* fileName ) -> void
	{
		blob = new CDnnBlob( MathEngine() );
		CArchiveFile file( fileName, CArchive::load );
		CArchive archive( &file, CArchive::load );
		blob->Serialize( archive );
	};
	auto analyze = [&blob] ( const char* name ) -> void
	{
		CDnnBlobBuffer<float> buff( *blob, TDnnBlobBufferAccess::Read );
		float maxValue = -100;
		float minValue = 100;
		for( int i = 0; i < buff.Size(); ++i ) {
			maxValue = max( maxValue, buff[i] );
			minValue = min( minValue, buff[i] );
		}
		::printf( "*** %s ***\n", name );
		::printf( "values are in [ %.10f (%#010x) ; %.10f (%#010x) ]\n",
			minValue, *reinterpret_cast<uint32_t*>( &minValue ), maxValue, *reinterpret_cast<uint32_t*>( &maxValue ) );
	};
	load( "softmax.dnnblob" );
	analyze( "ARM64 SOFTMAX" );
	MathEngine().VectorLog( blob->GetData(), blob->GetData(), blob->GetDataSize() );
	analyze( "LOG" );
	load( "logarithm.dnnblob" );
	analyze( "ARM64 LOG" );
}
...
TEST( DebugArm64, LogApproximationNear1 )
{
	...
	CArray<float> outputData;
	outputData.SetSize( inputData.Size() );

	const float one = 1.f;
	const int oneBits = *reinterpret_cast<const int*>( &one );
	for( int x = oneBits - delta; x <= oneBits + delta; ++x ) {
		inputData[x - oneBits + delta] = *reinterpret_cast<const float*>( &x );
	}

	MathEngine().VectorLog( CARRAY_FLOAT_WRAPPER( inputData ), CARRAY_FLOAT_WRAPPER( outputData ), dataSize );
	...
	::printf( "log( 0.f ) is %.10f   // hex output: %010x\n",
		outputData[delta], *reinterpret_cast<const uint32_t*>( &inputData[delta] ) );
