...
В целях отладки `IBytePairEncoder` предоставляет прямые методы загрузки и выгрузки словаря (`LoadDictionary` и `GetDictionary`). Словарь, созданный не алгоритмами NeoML, должен соотве:
