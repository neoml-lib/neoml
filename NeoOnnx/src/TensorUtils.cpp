...
CPtr<const CTensorBase> ConvertTensor( const CTensorBase& input, const CTensorLayout& outputLayout )
{
	...
	CTensorLayoutMatchFunctor functor( outputLayout );
	CTensorLayout result = FindOptimalConversion( input.Layout(), functor, renameBefore, transposes, renameAfter );
	NeoAssert( result == outputLayout );
