...
// The micro-kernel size is 8*1. The temporary variables are stored in q3 and q4 registers
// The A input matrix is stored row-by-row
// When iterating by K, four values from B are written into q0
// When iterating by K, each column from A is written into q1 and q2
...
// |    | 
// v    v 
struct CMicroKernel8x1 : public CMicroKernelBase<8, 1> {
	static void Calculate(const float* aPtr, const float* bPtr, float* cPtr, size_t ldc, size_t k)
	{
		...
		__asm __volatile(
			...
			// then transferring them into one vector register
			"ldr	s3,    [%[cLdr], #0x0]\n"
			"add    %[cLdr],%[cLdr], %[ldc]\n"
			"ldr	s2,    [%[cLdr], #0x0]\n"
			"add    %[cLdr],%[cLdr], %[ldc]\n"
			...
			"bls 2f\n"
			// The main cycle over K (4 at a time)
		"1:\n"
			"ldr    q0,     [%[bPtr], #0x0]\n"
			"ldr    q1,     [%[aPtr], #0x0]\n"
			"ldr    q2,     [%[aPtr], #0x10]\n"
			...
			"ldr    q1,     [%[aPtr], #0x40]\n"
			"fmla   v4.4s,  v2.4s, v0.s[1]\n"
			"ldr    q2,     [%[aPtr], #0x50]\n"

			"fmla   v3.4s,  v1.4s, v0.s[2]\n"
			"ldr    q1,     [%[aPtr], #0x60]\n"
			"fmla   v4.4s,  v2.4s, v0.s[2]\n"
			"ldr    q2,     [%[aPtr], #0x70]\n"

			"fmla   v3.4s,  v1.4s, v0.s[3]\n"
			"fmla   v4.4s,  v2.4s, v0.s[3]\n"
		...
		"3:\n"
			...
			// is split into several and then saved
		"4:\n"
			"dup    s2,      v3.s[1]\n"
			"dup    s1,      v3.s[2]\n"
			"dup    s0,      v3.s[3]\n"
