...
--*/
{
    ...
    if (M == 1 && TransA == MlasNoTrans && alpha == 1.0f && (beta == 0.0f || beta == 1.0f)) {

#if defined(MLAS_TARGET_AMD64)

        MLAS_SGEMM_KERNEL_M1_ROUTINE* SgemmKernelM1Routine;

        if (TransB == MlasNoTrans) {
...
MlasSgemmThreaded(
    ...
    const MLAS_TRANSPOSE TransA,
    const MLAS_TRANSPOSE TransB,
    const size_t M,
    const size_t N,
    const size_t K,
...
--*/
{
    ...
    const float* A = DataParams->A + RangeStartM * ((TransA == MlasNoTrans) ? lda : 1);
    ...
    if (DataParams->BIsPacked) {
        ...
        const float* B = (const float*)DataParams->B + RangeStartN * ((TransB == MlasNoTrans) ? 1 : ldb);
