	...
	const CFloatHandle& resultHandle, int vectorSize )
{
	...
	if( sseSize > 0 ) {
		...
		for( int i = 0; i < sseSize; ++i ) {
			...
			__m128 middlePart = _mm_cmplt_ps( minusThreeSse, firstSSE );
			__m128 secondSSE = _mm_loadu_ps( second );
			middlePart = _mm_and_ps( middlePart, _mm_mul_ps(secondSSE, _mm_add_ps( _mm_mul_ps( firstSSE, oneThirdSse ), halfSse ) ) );
			__m128 rightPart = _mm_cmpge_ps( firstSSE, threeSse );
			_mm_storeu_ps( result, _mm_blendv_ps( middlePart, secondSSE, rightPart ) );
