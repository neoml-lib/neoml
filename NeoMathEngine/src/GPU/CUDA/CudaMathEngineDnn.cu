...
CActivationDesc* CCudaMathEngine::InitActivation( const CActivationInfo& activation, int dataSize )
{
	size_t bufferSize = 0;
...
bool CCudaMathEngine::IsInPlaceActivation( TActivationFunction activation )
{
	return activation != AF_Abs && activation != AF_HSwish && activation != AF_GELU;
}

void CCudaMathEngine::Activation( const CActivationDesc& activationDesc, const CConstFloatHandle& input,
...
void CCudaMathEngine::ActivationBackward( const CActivationDesc& activationDesc, const CConstFloatHandle& input,
