...
static void multiplyTransposedLookupMatrixByVectorTestImpl( const CTestParams& params, int seed )
{
...

	{
		...
		get.resize( batchSize * width );
		expected.insert( expected.begin(), batchSize * width, 0 );
...
class CMultiplyTransposedLookupMatrixByVectorTest : public CTestFixtureWithParams {
};

INSTANTIATE_TEST_CASE_P( CMultiplyTransposedLookupMatrixByVectorTestInstantiation, CMultiplyTransposedLookupMatrixByVectorTest,
	::testing::Values(
		CTestParams(
			"Height = (1..50);"
			"Width = (1..50);"
