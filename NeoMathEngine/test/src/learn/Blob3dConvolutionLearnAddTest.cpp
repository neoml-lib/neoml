...
struct CConv3dLearnAddParams {
	...
	CConv3dLearnAddParams( int inputCount, int inputHeight, int inputWidth, int inputDepth, int inputChannels,
...
static inline float* getInputElem( const CConv3dLearnAddParams& params, int h, int w, int d, int ch, float* input )
{
...
static void calcOneStepConvolutionBackward( const CConv3dLearnAddParams& params, int fromH, int fromW, int fromD, float* input,
...
static void convolutionBackward( const CConv3dLearnAddParams& params,
...
static void batchConvolutionBackward( const CConv3dLearnAddParams& params,
	float *input, float *filter, float *freeTerm, float *output )
{
	const int outputHeight = calcConvOutputSize( params.InputHeight, params.PaddingHeight, params.FilterHeight, params.StrideHeight );
	const int outputWidth = calcConvOutputSize( params.InputWidth, params.PaddingWidth, params.FilterWidth, params.StrideWidth );
...
static CConv3dLearnAddParams getConv3dParams( const CTestParams& params, CRandom& random )
{
	...
	return CConv3dLearnAddParams( batchSize, inputHeight, inputWidth, inputDepth, inputChannels,
...
static void blob3dConvolutionLearnAddImpl( const CTestParams& params, int seed )
{
	...
	MathEngine().Blob3dConvolutionLearnAdd( *convDesc, inputBlob.GetData(), CFloatHandle(), outDiffBlob.GetData(),
		filterDiffBlob.GetData(), addFreeTerm ? &ft : nullptr, false );
	delete convDesc;

	filterDiffBlob.CopyTo( filterDiff.data() );

	batchConvolutionBackward( convParams, inputData.data(), expectedFilterDiff.data(), expectedFreeTermDiff.data(), outputDiff.data() );
