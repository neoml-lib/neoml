...
static void addWidthIndexImpl( const CTestParams& params, int seed )
{
	...
	MathEngine().AddWidthIndex( inputBlob.GetDesc(), inputBlob.GetData(), true, outputBlob.GetData() );

	addIndexNaive( inputData.data(), batchSize, height, width, channels, outputData.data(), false );
	...
	MathEngine().AddWidthIndex( inputBlob.GetDesc(), outputBlob.GetData(), false, inputBlob.GetData() );
