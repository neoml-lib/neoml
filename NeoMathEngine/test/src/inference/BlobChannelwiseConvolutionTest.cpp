...
class CTimeVector : public std::vector<IPerformanceCounters::CCounter::TCounterType>
{
public:
	~CTimeVector()
	{
...
static void blobChannelwiseConvolutionTestImpl( const CTestParams& params, int seed )
{
	...
	std::unique_ptr<IPerformanceCounters> timer( MathEngine().CreatePerformanceCounters() );
	timer->Synchronise();
	...
	timer->Synchronise();
	timeVec.push_back( ( *timer )[0].Value );
	delete convDesc;
