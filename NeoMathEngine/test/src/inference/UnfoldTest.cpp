...
static inline void unfoldNaive( const std::vector<float>& images, std::vector<float>& matrices,
...
{
	ASSERT_EQ( static_cast<int>( images.size() ), batchSize * imageHeight * imageWidth * channels );
	...
	matrices.resize( 0 );
	matrices.resize( batchSize * matrixHeight * matrixWidth, 0 );
					...
					for( int filterX = 0; filterX < filterWidth; ++filterX ) {
						...
						if( inX >= 0 && inY >= 0 && inX < imageWidth && inY < imageHeight ) {
							...
							for( int c = 0; c < channels; ++c ) {
								matrices[outIndex++] = images[inIndex++];
							}
						} else {
							outIndex += channels;
						}
...
static void unfoldTestImpl( const CTestParams& params, int seed )
{
	CRandom random( seed );
	...
	CREATE_FILL_FLOAT_ARRAY( inputData, valuesInterval.Begin, valuesInterval.End, inputBatch * inputHeight * inputWidth * inputChannels, random )
	CFloatBlob inputBlob( MathEngine(), 1, inputBatch, 1, inputHeight, inputWidth, 1, inputChannels );
	inputBlob.CopyFrom( inputData.data() );

	CFloatBlob outputBlob( MathEngine(), 1, inputBatch, 1, outputHeight * outputWidth, 1, 1, filterHeight * filterWidth * inputChannels );

	MathEngine().Unfold( inputBatch, inputBlob.GetData(), inputHeight, inputWidth, inputChannels,
		outputBlob.GetData(), filterHeight, filterWidth, strideHeight, strideWidth,
		paddingHeight, paddingWidth, dilationHeight, dilationWidth );

	std::vector<float> actual( outputBlob.GetDataSize() );
	outputBlob.CopyTo( actual.data() );

	std::vector<float> expected;
	unfoldNaive( inputData, expected, inputBatch, inputHeight, inputWidth, inputChannels,
...
class CMathEngineUnfoldTest : public CTestFixtureWithParams {
};

TEST_P( CMathEngineUnfoldTest, Random )
{
	RUN_TEST_IMPL( unfoldTestImpl );
}

INSTANTIATE_TEST_CASE_P( CMathEngineUnfoldTestInstantiation, CMathEngineUnfoldTest,
	::testing::Values(
		CTestParams(
			"InputBatch = (1..3);"
			"InputHeight = (10..15);"
