...
// Some MathEngine methods may need int or float value to calculate something
// Using this wrapper you could transfer to device kernel either host value or
// if you already have a result in device memory you can transfer it without any 
// data syncronizations with device
template <typename T>
class CScalarParameter final {
public:
	CScalarParameter( T value ) : Value( value ) {}
	CScalarParameter( CTypedMemoryHandle<const T> handle ) : Handle( handle ) {}
	// Converters
	CScalarParameter( CTypedMemoryHandle<T> handle ) : Handle( handle ) {}
	CScalarParameter( const CMemoryHandleVar<T>& var ) : Handle( var.GetHandle() ) {}
	CScalarParameter( const CMemoryHandleVar<const T>& var ) : Handle( var.GetHandle() ) {}
	CScalarParameter( const CMemoryHandleStackVar<T>& var ) : Handle( var.GetHandle() ) {}
	CScalarParameter( const CMemoryHandleStackVar<const T>& var ) : Handle( var.GetHandle() ) {}
	
	// For CUDA instead of convert to value use CCudaScalarParameter to use the value on device
	operator T() const { return Handle.IsNull() ? Value : ( Handle.GetValue() /*CUDA or Vulkan sync*/ ); }

	// Either the value or device memory value in the handle
	const T Value{};
	const CTypedMemoryHandle<const T> Handle{};
};

using CFloatParam = CScalarParameter<float>;
using CIntParam = CScalarParameter<int>;
...
public:
	...
	virtual void VectorEqualValue( const CConstIntHandle& firstHandle,
		const CFloatHandle& resultHandle, int vectorSize, CIntParam value ) = 0;

	// result = max( first, second )
	virtual void VectorMax( const CConstFloatHandle& firstHandle, CFloatParam secondValue, const CFloatHandle& resultHandle,
		int vectorSize ) = 0;
	virtual void VectorMaxDiff( const CConstFloatHandle& firstHandle, CFloatParam secondValue, const CFloatHandle& gradHandle,
		int gradHeight, int gradWidth ) = 0;

	// ELU
	virtual void VectorELU(const CConstFloatHandle& firstHandle, const CFloatHandle& resultHandle,
		int vectorSize, CFloatParam alpha) = 0;
	virtual void VectorELUDiff(const CConstFloatHandle& firstHandle, const CConstFloatHandle& secondHandle,
		const CFloatHandle& resultHandle, int vectorSize, CFloatParam alpha) = 0;
	...
	virtual void VectorReLU(const CConstFloatHandle& firstHandle, const CFloatHandle& resultHandle, int vectorSize,
		CFloatParam upperThreshold) = 0;
	virtual void VectorReLUDiff(const CConstFloatHandle& firstHandle, const CConstFloatHandle& secondHandle,
		const CFloatHandle& resultHandle, int vectorSize, CFloatParam upperThreshold) = 0;
	...
	virtual void VectorHardSigmoid(const CConstFloatHandle& firstHandle, const CFloatHandle& resultHandle, int vectorSize,
		CFloatParam slope, CFloatParam bias) = 0;
	virtual void VectorHardSigmoidDiff(const CConstFloatHandle& firstHandle, const CConstFloatHandle& secondHandle,
		const CFloatHandle& resultHandle, int vectorSize, CFloatParam slope, CFloatParam bias) = 0;
	...
	virtual void VectorBernulliKLDerivative(const CConstFloatHandle& estimationHandle,
		const CFloatHandle& resultHandle, int vectorSize, CFloatParam target) = 0;
	...
	virtual void VectorAddValue(const CConstFloatHandle& firstHandle,
		const CFloatHandle& resultHandle, int vectorSize, CFloatParam addition) = 0;
	virtual void VectorAddValue( const CConstIntHandle& firstHandle,
		const CIntHandle& resultHandle, int vectorSize, CIntParam addition ) = 0;
	...
	virtual void VectorMultiplyAndAdd(const CConstFloatHandle& firstHandle, const CConstFloatHandle& secondHandle,
		const CFloatHandle& resultHandle, int vectorSize, CFloatParam mult) = 0;
	// result = first - mult * second
	// You may NOT pass the same handle as secondHandle and resultHandle parameters
	...
	virtual void VectorMultiply(const CConstIntHandle& firstHandle,
		const CIntHandle& resultHandle, int vectorSize, CIntParam mult) = 0;
	// result = -first * multiplier
	...
	virtual void VectorMinMax(const CConstFloatHandle& firstHandle, const CFloatHandle& resultHandle, int vectorSize,
		CFloatParam min, CFloatParam max) = 0;
	...
	virtual void VectorL1DiffAdd(const CConstFloatHandle& firstHandle, const CConstFloatHandle& secondHandle,
		const CFloatHandle& resultHandle, int vectorSize, CFloatParam hubertThreshold, CFloatParam mult) = 0;
...
public:
	...
	virtual void VectorMultichannelLookupAndAddToTable(int batchSize, int channelCount, const CConstFloatHandle& inputHandle,
		...
		CFloatParam mult, const CConstFloatHandle& matrixHandle, int outputChannels) = 0;
